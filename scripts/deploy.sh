...
aws s3 cp s3://www.funnyhowtheknightmoves.com/sw.js s3://www.funnyhowtheknightmoves.com/sw.js --metadata-directive REPLACE --cache-control max-age=0,no-cache,no-store,must-revalidate --content-type application/javascript --acl private
...
aws cloudfront create-invalidation --distribution-id $distribution_id --paths /index.html /sw.js
